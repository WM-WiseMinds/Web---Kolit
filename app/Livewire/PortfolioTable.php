...
final class PortfolioTable extends PowerGridComponent
{
    ...
    public function actions(Portfolio $row): array
    {
        ...
        if (auth()->user()->can('update')) {
            $actions[] =
                ...
                ->openModal('portfolio-form', ['rowId' => $row->id]);
        }
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.portfolio', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/portfolio.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/portfolio.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $portfolio = Portfolio::findOrFail($rowId);

        if ($portfolio->gambar) {
            Storage::disk('public')->delete($portfolio->gambar);
        }
        $portfolio->delete();
        $this->success('Portfolio berhasil dihapus');
    }
}
